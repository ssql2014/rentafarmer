...
AI 输出推荐品种、面积、费用预估、周期、服务配置和微信承接话术。默认服务特色是有机方式管护、不打农药、24 小时视频监控、当地农民提供服务。瓜果成熟后，用户可以自行采摘、邮寄到家，或委托接口人代为销售。
...
当前版本不直接引导支付，先让人工确认地块、农时、视频监控、配送、成熟后处理方式和合同边界。有机方式管护不等同于有机产品认证；如果用户要求认证级有机产品，需要接口人核验证书。
